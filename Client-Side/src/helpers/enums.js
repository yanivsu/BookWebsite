import Pic1 from "../styles/bookPictures/BookInfo1.jpg";
import Pic2 from "../styles/bookPictures/BookInfo2.png";
import Pic3 from "../styles/bookPictures/BookInfo3.jpg";
import Pic4 from "../styles/bookPictures/BookInfo4.png";

export const buttonsText = {
  BUY: "לרכישה",
  CREATION: "יצירה",
  ABOUT: "אודות",
  GALLARY: "גלריה",
  BOOKPAGE: "תמונת הספר",
  PROFILEPICTURE: "תמונת פרופיל",
  BUY_LINK:
    "https://www.meshulam.co.il/purchase?b=25b22fc8b8fc81a9741b428fb65d7c6f",
};

export const aboutComponentsTexts = {
  TITLE: "הליצן שפחד לרכב על חד-אופן",
  PARAGRAPH:
    "מי לא אוהב לקבל מתנות ?\n  \n אז מה הוביל את לירן לכעוס כל כך על מתנה שקיבל?\n כיצד פעל ואיך סייעו לו חבריו לתקן את המצב?\n\n הליצן שפחד לרכוב על חד-אופן הוא סיפור על כוחה של חברות, על הכרת הכוחות שבנו ושימושם בתחומים שונים. זהו סיפור על התגברות על פחד - באומץ, בנחישות ובהתמדה.",
};

export const artInfoTexts = {
  LINOY: "לינוי אסולין",
  ABOUT_LINOY:
    "אוהבת לטייל, מילים ופאזלים. היא מנסה לגדל צמחים, מוצאת יופי במתמטיקה ומאמינה גדולה בחינוך ובאנשים.",
  NOA: "נועה מושיוב",
  ABOUT_NOA: `אוהבת ללכת לשון מוקדם, לטייל בטבע, לצייר וללמוד. גם היא אוהבת ממתקים, כלבים, הרפתקאות ומתמטיקה.`,
};

export const paintPapersLinks = [
  "https://lh6.googleusercontent.com/utEJ0r9jJiqPjT5m4EANbwFfq36Bz0A7plu_1KoSpn1fAiDaygt3-K0Fay4qFgEJWtk1S_IHoryXEdlH6kodmn81vZi6km0_pq7bZQ7MavBmoifcVMznpSZauIpxKX8IwsEBWutM",
  "https://lh4.googleusercontent.com/rWgXfOiDL8Dr6L7HZ2YyAa0wl7Av3uCbor97RN3iuBpNHJu3hQ4I9XKEC9pioVjID2XXrdtgfVNP5Q6CRs1o-0NAByaXQX39Waga0D6J5Ovbo_6sRCWFi3aTcLUr5IZubo0r6EFV",
  "https://lh4.googleusercontent.com/WABccsTQBBwuqLnUxVh_IB2ja2AIuE9uHXxax9S4wOlPeP5usybO7M4apkhbE6Nels8N-pY7uUXCCoeE36PqgAteoBkEnYht5466lP7iMsKR9pw_Nl9z9pby6VwoUwwCeIrQG2tK",
  "https://lh6.googleusercontent.com/g5gKQqkAzY5Xja50_MMyWW2Ojn0Z7fMb0e8-WEFcimoLs-cGxpPqIdKuV0bRoWT7EWkAkuOduYh9164yzS_sMl3F3KI6jCGZ4PAhthmrlVho1-wgh98VlNNSYKbWScmvIluSR3cu",
  "https://lh4.googleusercontent.com/Y8lrDOYH_LNbtOHSVaryWx_Lnbm_6RtBZ6of3cQfukLTjlOZ2nhtY6ZwlTenepogotJZk445myXJTNGXt1xMv6Tdxd1eJYfLErYd9GBkQOwxvQ9iC3EIkRLITqXVfCe9pHARPpJT",
  "https://lh4.googleusercontent.com/xoj_0i50iTXCC9-jezL_Zb3O7vqFpYB4WSmZjaCFLs2e7ZB6dwrF24SIk8bhNDp5Q7ZNGEL80UiLwiXihJ0Fiot9AJFYXKkxL8BKuq0GFPvFbf5XkzeEbXQeNjM9CkcsdLj85dvC",
];

export const bookPictursLinks = [Pic1, Pic2, Pic3, Pic4];

export const bookInfoParag = `למדתי לרכוב על אופניים בגיל 20. עד כאן, סיפור פשוט יחסית.  רק ש - עבורי זה היה רגע מכונן בהחלט. אני זוכרת את הרגע הזה בדיוק רב, הרגע שבו רגל  אחת מובילה את השנייה וזה קורה, את דפיקות הלב הנרגשות והתחושה שניצחתי.  (כשקיבלתי מנועה המאיירת את עמוד הניצחון של לירן התחושה הכתה בי שוב, כאילו השמש יצאה ממני ברגע הזה, ממש כמו באיור, בלי שום תיאום מוקדם שלנו)

אחי נהרג כשאני הייתי בת 8, הוא יצא לסיבוב בשכונה על האופניים ולא שב. מאז זוג האופניים היחיד שנותר בבית היה שלו - מצבת זיכרון עבורו. ואני, ניסיתי כמה פעמים בסמוך , לעלות על אופניים, לרכוב, במסגרת כל מיני פעילויות בית ספריות שכאלה.  זה לא צלח ולא מנע ממני יותר מדי - הנחתי את זה בצד.

במרוץ הימים, עבדתי תקופה בהולנד, ושם - כולם רוכבים, אז רציתי גם.

אני זוכרת את הרגע הזה בדיוק רב, אותו רגע שהצלחתי. לצדו אני זוכרת בדיוק רב גם נסיעה בקטע כביש קצר, כשאני רוכבת עם כיוון התנועה ושומעת רכב מתקרב אליי, אני זוכרת את הבהלה שאחזה בי. את המחשבות והשאלות שצפו באותו הרגע , שבריר שניה. אני זוכרת את אלו ואני זוכרת את ההחלטה לרכוב שוב, כי יש ברירה - אבל היא לא טובה מספיק. 

באחד המכתבים שכתבתי לרונאל,  אחי, הבטחתי להקדיש לו מעשיית הטוב , והרבה ממנה. הספר שכתבתי הוא לכבודו, לזכרו, לאורו.

אין זה סיפור על מוות, זה סיפור על חיים. הבאתי בסיפור הזה את עולמי הפרטי, את מה שאני מוצאת כמשמעותי וחשוב בחייו של אדם, מתוך ניסיוני החינוכי מזה שנים, מתוך מה שהבנתי שאני צריכה.

בסיפור מובא בבירור הצורך שלנו בסביבה, בכוחה של הסביבה להאיר את חיינו ולהאיר את תכונותינו החיוביות, אנחנו אולי יכולים לבד, אבל טוב יותר ביחד. הסיפור שוזר בתוכו שימוש בתכונות ויכולות שכבר קיימות בנו ומאפשרות לנו לצמוח במקומות חדשים. זהו סיפור על חצייה של פחדים - כי החיים מלאים בהם. זהו סיפור על נחישות ועל התמדה ועל תרגול. בסופו של יום, הם אלה שמכתיבים את חיינו. 
 
אין בו רע,  אין בו אתגר שאינו מושג. יש בו מהיומיום שלנו, של כולנו - ילדים ובטח שמבוגרים.

זכיתי שיזכירו לי במסגרת סדנת התפתחות שעברתי, שלחצות אתגר זה כמו לרכוב על אופניים - כשזה שם, זה כבר שלך. הגוף זוכר, המח זוכר.

את כל אלה ועוד, ארזנו במילים, ארזנו באיורים ויצרנו מוצר אישי, רגיש, בהיר. תשומת הלב בכל אות ובכל נקודה וכולי תקווה להפיץ את הסיפור ומסריו לכל מי שימצא בו כוח .`;
